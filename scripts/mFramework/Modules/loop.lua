---@class loop
---@see https://github.com/SinisterRectus/LuaThreads
---@see https://github.com/SinisterRectus/LuaThreads/blob/master/LICENSE
...
function Thread:sleep(delay)
    self:stop()
    loop.setTimeout(delay, function() self:start() end)
end

function Thread:start() self.active = true end

function Thread:stop() self.active = false end

function Thread:clear() threads[self.coro] = nil end

function Thread:isActive() return self.active == true end

-- constructs and registers a new loop thread
local function newThread(f)
    local thread = setmetatable({coro = coroutine.create(f)}, Thread)
    threads[thread.coro] = thread
    return thread
end

-- returns the current process time in milliseconds
local function getTime() return os.clock() * 1000 end
...
function loop.runOnce()
    ...
    for coro, thread in pairs(threads) do
        ...
        if status == 'suspended' then
            running = true
            if thread:isActive() then assert(coroutine.resume(coro)) end
...
function loop.getThread(coro) return threads[coro or coroutine.running()] end
...
function loop.setTimeout(delay, callback)
    local thread = newThread(
            ...
            while true do
                local current = getTime()
                local dt = current - t
                if dt >= delay then return callback() end
                coroutine.yield()
            end
        end
                   )
