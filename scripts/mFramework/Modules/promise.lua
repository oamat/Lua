...
transition = function(promise, state, value)
  if
    promise.state == state or promise.state ~= State.PENDING or (state ~= State.FULFILLED and state ~= State.REJECTED) or
      value == nil
   then
    return
  end
...
--- Theros: Added Built in "thread" to poll Promise.update() for use with miscreated,
--- edit SchedualUpdateThread() func below for other implementations.

-- default thread update timer in seconds (default: 2)
Promise.UpdateDelay = 2

local promise_thread_started = nil
local promise_thread = function(self)
  if (promise_thread_started ~= true) then
    promise_thread_started = true
  end
  if mF_DEBUG then
    Log('Promise Thread Running...')
  end
  self.update()
  self:SchedualUpdateThread(self.UpdateDelay, 111)
end
--- Schedual ThreadUpdates for the Promise Library
function Promise:SchedualUpdateThread(updateDelay, int)
  local thread = promise_thread
  if (promise_thread_started ~= true) or (int == 111) then
    Script.SetTimerForFunction(((updateDelay or self.UpdateDelay) * 1000), 'thread', self)
  end
end
--- Start Updating Promise Thread
function Promise:Init()
  self:SchedualUpdateThread(1)
end

RegisterModule('mFramework.Promise', Promise)
return Promise
