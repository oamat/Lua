--[[
    !NOTE
    >> This Contains a Modified Version of the async/await tasks api found over at: https://gist.github.com/Meorawr/6a69ad8857a3fb7202fda9e8c2731fed
    >> i take no creadit for those parts of this file, some modifications have been made but its mostly their work. the extra parts should work mostly the same
    >> but only the Intended async/await function this exports and the DynamicServer TaskManager Object have been tested.
...
local function thread_main() while true do co_yield(invoke(co_yield())) end end
...
local function task_stack_top() return task_stack[#task_stack] end

-- Pushes the given task to the top of the stack.
local function task_stack_push(task) task_stack[#task_stack + 1] = task end

-- Pops the task at the top of the stack.
local function task_stack_pop() task_stack[#task_stack] = nil end
...
local function task_notify(task)
    if not task.notify then return end
...
local function task_main(task) return TASK_SUCCESS, invoke(unpack(task.target, 1, task.target.n)) end
...
local function task_postresume(task, ok, result, ...)
    ...
    if not ok or result == TASK_SUCCESS then
        ...
        if result == TASK_SUCCESS then thread_release(task.thread) end
...
local function task_join(task)
    ...
    if task.state == 'dead' then return end
...
local function task_schedule_all()
    ...
    while next(task_runnable) do
        for task in pairs(task_runnable) do runnables[#runnables + 1] = task end
...
function async(fn)
    return function(...)
        local modified_env = debug.getfenv(fn)
        modified_env['task_yield'] = task_yield
        debug.setfenv(fn, modified_env)
...
function AsyncTasks:Create(name, cb, updateRate)
    local function thisThread()
        local LastRun = 0
        return function()
            local updateRate = (updateRate or 3)
            local timeNow = os.clock()
            if (LastRun == 0) or (timeNow - LastRun > updateRate) then
                mFramework.Debug('DynamicServer Executing Callback Task:' .. name)
                local ok, cb_res = pcall(cb)
                if not ok then
                    mFramework.Debug('Callback Task:' .. name .. ' -> Failed. Result: ' .. tostring(cb_res or 'No Result'))
                else
                    mFramework.Debug('Callback Task:' .. name .. ' -> Done. Result: ' .. tostring(cb_res or 'No Result'))
                end
                LastRun = os.clock()
            end
        end
    ...
    local thisTask = {name = name, thread = thisThread()}
    InsertIntoTable(self.Tasks, thisTask)
    return thisTask
end

function AsyncTasks:Delete(task)
    local thisTask = FindInTable(self.Tasks, 'thread', task.thread)
    if thisTask then
        if (not thisTask.name == task.name) then return nil, 'Task to Delete must have the same Name' end
        RemoveFromTable(self.Tasks, task)
    end
    if (not IsInsideTable(self.Tasks, task)) then return true end
end

function AsyncTasks:RunTasks()
    for idx, task in ipairs(self.Tasks) do
        if (type(task) == 'table') and (type(task.thread) == 'function') then
            mFramework.Debug(string.format('AsyncTasks Running Task[%s] %s', tostring(idx), tostring(task.name)))
            local taskOk, taskResult = pcall(task.thread)
            if (not taskOk) then
                mFramework.Log(
                    string.format('AsyncTask Error Task[%s] %s > %s', tostring(idx), tostring(task.name), tostring(taskResult))
                )
            end
        end
    end
...
function DynamicServer:SchedualUpdateThread(updateDelay)
    ...
    if thread_Run then -- is thread enabled
        ...
        self.TimerId = Script.SetTimerForFunction((updateDelay * 1000), 'DynamicServer_thread', self)
    end
end

--- Starts this Thread, if thread allready started does nothing
function DynamicServer:Start()
...
DynamicServer_thread = function(self) -- the self is required for accessing the threads properties.
    task_schedule() --- ignore this its for mFrameworks async/promise stuff
    --- is this thread enabled?
    if thread_Run then
        local timeNow = os.clock() --- just checking enough time,
        if (LastRun == nil) or (timeNow - LastRun > updateRate) then --  has passed since last run
            --- Heres your threads main function idealy just make this call a callback or anouther function
            --- that does the hard work.
            mFramework.Debug('DynamicServer AsyncThread Running')
            AsyncTasks:RunTasks()
        ...
        self:SchedualUpdateThread(self.updateDelay)
        thread_Status = 1
    end
end